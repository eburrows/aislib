 ...
 *
 * Usage: java com.erikburrows.aisparselib.nmea2mysql -serial <device> -mysqlurl <url> -speed <speed> -mysqluser <user> -mysqlpassword <password> [-debug]
 */
public class serial2mysql {

    /**
     * Display command-line usage
     */
    public static void usage() {
	System.out.println("Usage: java com.erikburrows.aisparselib.nmea2mysql -serial <device> -speed <speed> -mysqlurl <url> -mysqluser <user> -mysqlpassword <password> [-debug]");
	...
	String serial_device = "/dev/ttyS0";
	...
	for (int i = 0; i < args.length; ++i) {
	    if (args[i].equalsIgnoreCase("-serial")) {
		if (args.length > i + 1) {

		    serial_device = args[i + 1];

		    i = i + 1;
		} else {
		    System.out.println("No serial device specified.");
		}

	    } else if (args[i].equalsIgnoreCase("-speed")) {
		if (args.length > i + 1) {

		    serial_speed = Integer.parseInt(args[i + 1]);

		    i = i + 1;
		} else {
		    System.out.println("No serial device speed specified.");
		}
	...
	if (debug)
	    System.out.println("Reading from " + serial_device + " at " + serial_speed + ", logging to " + mysql_url);
	...
	while(con == null) {
        ...
        CommPortIdentifier portIdentifier = CommPortIdentifier.getPortIdentifier(serial_device);
	CommPort commPort = portIdentifier.open("serial2mysql",2000);
	SerialPort serialPort = (SerialPort) commPort;
	serialPort.setSerialPortParams(serial_speed,SerialPort.DATABITS_8,SerialPort.STOPBITS_1,SerialPort.PARITY_NONE);
	...
	gpsdBufferedReader = new BufferedReader(new InputStreamReader(in, "US-ASCII"));
	gpsd_output_stream = new PrintStream(out);
